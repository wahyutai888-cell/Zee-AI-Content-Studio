...
with tab1:
    ...
    with col_kiri:
        ...
        prod_info = st.text_area("Detail/Link Produk:", placeholder="Contoh: Sepatu lari Nike Air Max, bahan breathable, diskon 50% di Shopee...", height=150, key="input_teks_produk")
        
        c1, c2 = st.columns(2)
        with c1:
            gaya = st.selectbox("Gaya Bahasa:", ["Persuasif/Mengajak", "Review Jujur", "Hard Sell", "Storytelling"], key="input_gaya")
        with c2:
            panjang = st.select_slider("Panjang Teks:", options=["Singkat", "Menengah", "Panjang"], key="input_panjang")
        
        # Penambahan KEY unik pada tombol
        if st.button("🚀 Craft My Caption", key="btn_craft_caption"):
            if not prod_info:
    ...
    with col_kanan:
        ...
        if st.session_state.hasil_caption:
            ...
            buffer.seek(0)
            # Penambahan KEY unik
            st.download_button("📥 Download to Word (.docx)", data=buffer, file_name="script_affiliate_zee.docx", key="btn_download_word")
        else:
...
with tab2:
    ...
    uploaded_file = st.file_uploader("Pilih Foto Produk", type=['png', 'jpg', 'jpeg'], key="input_gambar")
    if uploaded_file:
        ...
        with c_res:
            instruksi = st.text_input("Instruksi untuk AI:", "Berikan deskripsi produk yang estetik dan list fitur yang terlihat di foto ini.", key="input_instruksi_vision")
            # Penambahan KEY unik
            if st.button("🔍 Analisis Visual dengan AI", key="btn_analisis_vision"):
...
with st.sidebar:
    ...
    # Penambahan KEY unik
    if st.button("Cek Daftar Model", key="btn_cek_model"):
        try:
